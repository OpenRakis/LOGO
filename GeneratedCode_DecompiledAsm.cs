...
public class GeneratedOverrides_DecompiledAsm : CSharpOverrideHelper
{
    ...
    public virtual Action CircleMainLoop_1000_0CF4_10CF4(int loadOffset)
    {
        ...
        SI = 0xCBC;
        AX = UInt16[cs1, SI];
        UInt16[cs1, NumberOfFramesRemainingOffset_CB6] = AX;
        UInt16[cs1, 0xCBA] = SI;
        AX = 0;
        UInt16[cs1, 0xCB0] = AX;
        UInt16[cs1, 0xCB2] = AX;
    ...
    {
        SI = UInt16[cs1, 0xCBA];
        UInt16[cs1, NumberOfFramesRemainingOffset_CB6] = Alu16.Dec(UInt16[cs1, NumberOfFramesRemainingOffset_CB6]);
        if (ZeroFlag)
        {
            SI = Alu16.Add(SI, 8);
            UInt16[cs1, 0xCBA] = SI;
    ...
    public virtual Action CirclesAnimation_1000_0DDE_10DDE(int loadOffset)
    {
        ...
        do
        {
            ...
            do
            {
                ...
                if (!ZeroFlag)
                    return NearRet();
                int num = Alu16.Sub(UInt16[DS, 82], 0);
            }
            while (!ZeroFlag);
            SI = 0xEE;
            AX = UInt16[DS, SI];
    ...
    public virtual Action CirclesChangeVgaPaletteLoop_1000_0FA4_10FA4(int loadOffset)
    {
        ...
        while (true)
        {
            AX = UInt16[ES, SI];
            SI += (ushort)Direction16;
            int num = Alu8.Sub(AL, byte.MaxValue);
            if (!ZeroFlag)
            {
                CX = 0;
                BX = 0;
    ...
    public virtual Action HNMUnknown_1000_0FEA_10FEA(int loadOffset)
    {
        ...
        NearCall(cs1, 0xFF7, HNMUnknown_1000_0E46_10E46);
        BX = Stack.Pop16();
        BP = Stack.Pop16();
        BP >>= 1;
        BP >>= 1;
